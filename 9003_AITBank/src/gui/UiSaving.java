 ...
 * @author kosuk
 */
public class UiSaving extends BankUi {
...
//                textArea.repaint();
    ...
    public void showDetails() {
        String acFName = s.getFirstName();  //  get user first name
        String acLName = s.getLastName();   //  get user last name
        String acNum = Integer.toString(s.getAccountNumber());  // get an account number

        // set saving balance and get total balance
        Double balTotal = s.getSavingTotalBalance();
        String sBalTotal = String.format("%.2f", balTotal);

        // get a total withdraw limit of number
        Double balLimit = s.getSavingWithdrawalLimit(); 
        String sBalLimit = String.format("%.0f", balLimit);

        String regDate = ca.getRegSaving();   // get a registration date

        infoPanel.setText(
            "Account Name: " + acFName + " " + acLName + "\n" +
            "Account Number: " + acNum + "\n" +
            "Total Balance: " + sBalTotal + "\n" +
            "Today's whithdraw Limit: " + sBalLimit + "\n" +
    ...
    private void setNumberButtons() {
        ...
        btnEtr.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                switch (serviceChosen) {
                    case 1:
                        s.setWithdrawAction(dAmountNum);
                        System.out.println("withdraw action!");
                        continueAction();
                        break;
                    case 2:
                        s.setDepositAction(dAmountNum);
    ...
    private void setTransferButtons() {
        btnT1 = new JButton("Net Saving");
        btnT2 = new JButton("Cheque");
        btnT3 = new JButton("Fixed");
        
        // Transfer to net saveing --------- 
        btnT1.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                n.setTransferAddAction(dAmountNum);
                s.setTransferSubAction(dAmountNum);
                continueAction();
            }
        });
        ...
        btnT2.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                c.setTransferAddAction(dAmountNum);
                s.setTransferSubAction(dAmountNum);
                continueAction();
            }
        });
